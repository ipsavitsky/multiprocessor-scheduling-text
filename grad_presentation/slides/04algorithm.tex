...
\begin{frame}
    \frametitle{Общая схема жадных алгоритмов}
    {
        \small
        \begin{tikzpicture}
            \node (start) at (0, 0) [draw, terminal] {Старт};
            \node (decision) at (0, -2) [draw, decision, align=center] {Все работы добавлены \\ в расписание?};
            \node (finish) at (-4, -3) [draw, terminal] {Финиш};
            ...
            \draw[->] (start) -- (decision);
            \draw[->] (decision) -- node[right]{Нет} (choose_task);
            \draw[->] (choose_task) -- (choose_proc);
            \draw[->] (choose_proc) -- (add_task);
            \draw[->] (add_task) |- (decision);
            \draw[->] (decision) -| node[above]{Да} (finish);
        \end{tikzpicture}
    }
